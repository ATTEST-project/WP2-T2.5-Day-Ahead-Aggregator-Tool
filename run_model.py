...
def run_model_buildings(m, h, number_buildings, profile_solar, temp_outside, resources_agr,
                        buses_w, buses_g, buses_h, gen_dh, other_h):
    ...
    for t in range(0, h):
            ...
            if j + 1 not in buildings_with_dh:
                if j + 1 in buildings_with_hp_thermal:
                    ...
                    if t == 0:
                        ...
                        if building[j]['installed']['ac'] == 0:
                            m.c1.add(m.temp_building[j, t] == alpha * m.temp_building_up[j, t - 1] + (1 - alpha) *
                                 (temp_outside[t] + building_r * rend * (m.P_hp[j, t] + m.P_gb[j, t])))
                        else:
                            m.c1.add(m.temp_building[j, t] == alpha * m.temp_building_up[j, t - 1] + (1 - alpha) *
                                     (temp_outside[t] + building_r * rend * (m.P_hp[j, t] + m.P_gb[j, t]) -
                        ...
                        if building[j]['installed']['hp'] == 1:
                            if building[j]['installed']['ac'] == 0:
                                ...
                                m.c1.add(
                                    m.temp_building_up[j, t] == alpha * m.temp_building_up[j, t - 1] + (1 - alpha) *
                                    (temp_outside[t] + building_r * rend * (m.P_hp[j, t] - m.P_hp_up[j, t]) -
                                     building_r * rend_ac * (m.P_ac[j, t] - m.P_ac_up[j, t])))
                                m.c1.add(
        ...
        for j in range(0, number_buildings):
            if j + 1 in buildings_with_sto:
                ...
                m.c1.add(sum(m.P_sto_up[j, t] + m.P_sto_down[j, t] for t in range(t, h)) <= sum((P_max - m.P_sto_ch[j, t] - m.P_sto_dis[j, t])/2 for t in range(t, h)))
